...
graph_utils.delete_unused_edge_attrs(graph, save_attrs=['uvkey', 'length', 'geometry', 'noises', 'osmid'])
edge_dicts = graph_utils.get_all_edge_dicts(graph, attrs=['geometry'], by_nodes=False)
...
#%% 3.2 Remove unwalkable streets & tunnels from the graph [prepare graphs for comparison]
filt_edge_gdf = graph_utils.get_edge_gdf(graph_filt, by_nodes=True)
# add osmid as string to unwalkable (filter) edge gdfs
filt_edge_gdf['osmid_str'] = [graph_utils.osmid_to_string(osmid) for osmid in filt_edge_gdf['osmid'] ]
...
filt_edges_file.to_file('data/graphs.gpkg', layer=filt_edges_filename, driver="GPKG")
print('exported', filt_edges_filename, 'to data/graphs.gpkg')
# get edge gdf from graph
edge_gdf = graph_utils.get_edge_gdf(graph, by_nodes=True, attrs=['geometry', 'length', 'osmid'])
# add osmid as string to edge gdfs
edge_gdf['osmid_str'] = [graph_utils.osmid_to_string(osmid) for osmid in edge_gdf['osmid']]
...
all_edges_to_rm_gdf.drop(columns=['filter_match', 'uvkey', 'osmid']).to_file('data/graphs.gpkg', layer=rm_edges_filename, driver="GPKG")
print('\nexported', rm_edges_filename, 'to data/graphs.gpkg')
...
graph_utils.delete_unused_edge_attrs(graph)
...
for sb in sub_graphs:
    ...
    if (sub_graph_size >= 40 and sub_graph_size < len(graph.nodes)-5000):
        sub_g_edges = graph_utils.get_edge_gdf(sub_graph, attrs=['geometry', 'length'])
        sub_g_filename = graph_name +'_large_subgraph_n'+ str(sub_graph_size)
        sub_g_edges.drop(columns=['uvkey']).to_file('data/graphs.gpkg', layer=sub_g_filename, driver="GPKG")
        print('exported', sub_g_filename, 'to data/graphs.gpkg')
...
edge_dicts = graph_utils.get_all_edge_dicts(graph, attrs=['geometry', 'length'], by_nodes=False)
...
edge_noise_length_check = noise_exps.compare_lens_noises_lens(edge_sample)
...
if (len(odd_edge_gdf_noise_costs) > 0):
    ...
    odd_edge_gdf_noise_costs.drop(columns=['uvkey']).to_file('data/graphs.gpkg', layer=odd_edge_noise_costs_filename, driver="GPKG")
else:
