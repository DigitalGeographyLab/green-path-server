...
def convert_aq_nc_to_tif(
    ...
    aqi.rio.to_raster(fr'{dir}{aqi_tif_name}', dtype='float64')
    return aqi_tif_name
...
def fillna_in_raster(
    ...
    aqi_raster = rasterio.open(aqi_filepath, dtype='float64')
    ...
    for offset in [0.0, 0.01, 0.02, 0.04, 0.06, 0.08, 0.1, 0.12]:
        ...
        nodata_count = np.sum(aqi_band <= na_offset, dtype='float64')
        if log:
        ...
        if nodata_count > 180000:
            break
    if nodata_count < 180000 and log:
    ...
    invalid_count = np.sum(aqi_band_fillna < 1.0, dtype='float64')
    if invalid_count and log:
