...
def get_current_enfuser_key_filename() -> Tuple[str, str]:
    """Returns a key pointing to the current (expected) enfuser zip file in AWS S3 bucket. 
    Also returns a name for the zip file for exporting the file. The names of the key and the zip
    are based on the current UTC time string (e.g. 2019-11-08T11).
    """
...
class AqiFetcher:
    ...
    def fetch_process_current_aqi_data(self) -> None:
        ...
        aqi_nc_name = aq_processing.extract_zipped_aq_file(self.__aqi_dir, aqi_zip_name, 'allPollutants')
