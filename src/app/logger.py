...
class Logger:
    ...
    def debug(self, text: str):
        ...
        if self.app_logger: self.app_logger.debug(text)

    def info(self, text: str):
        if self.level >= 3: self.print_log(text, 'INFO')
        if self.app_logger: self.app_logger.info(text)

    def warning(self, text: str):
        if self.level >= 2: self.print_log(text, 'WARNING')
        if self.app_logger: self.app_logger.warning(text)

    def error(self, text: str):
        self.print_log(text, 'ERROR')
        if self.app_logger: self.app_logger.error(text)

    def critical(self, text: str):
        self.print_log(text, 'CRITICAL')
        if self.app_logger: self.app_logger.critical(text)
